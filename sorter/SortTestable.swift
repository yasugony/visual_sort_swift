...
open class BaseSorter : SortTestable {
    ...
    public func setCurrentDiffDataDump(_ index1: Int, _ index2: Int, _ dumpType:DumpType) {
        dumpDataArray.append("\"\(index1),\(index2),\(dumpType)\"")
    }
    ...
    public func swapDump(_ index1: Int, _ index2: Int) {
        swap(index1, index2)
        setCurrentDiffDataDump(index1, index2, DumpType.X)
    }
}
